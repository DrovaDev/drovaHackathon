		...
		business.profileEdit.useMutation({
			...
			onError: (error) => {
				const message = axios.isAxiosError(error)
					...
					: undefined;
				toast.error(
					message || "Unable to update business details. Try again.",
				);
			},
		});
						...
						<div className="flex items-start justify-between flex-wrap gap-4 mb-4">
							<div>
								...
								<p className="text-white/70 text-sm max-w-sm">
									...
									discover your business and submit delivery
									quotations — no account required.
								</p>
							</div>
							<a
								href={storefrontUrl}
					...
					<Link href="/dashboard/storefront/operations">
						<div className="bg-popover rounded-2xl border border-border p-5 cursor-pointer hover:border-secondary/40 hover:shadow-sm transition-all flex items-center gap-4">
							...
							<div>
								...
								<div className="text-xs text-muted-foreground mt-0.5">
									Hours, delivery scope, vehicle fleet,
									service types
								</div>
							</div>
							<MaterialIcon
